...
import static hs.jjm.FanOutOneBlockAsyncDFSOutputHelper.completeFile;
import static hs.jjm.FanOutOneBlockAsyncDFSOutputHelper.endFileLease;
import static hs.jjm.FanOutOneBlockAsyncDFSOutputHelper.getStatus;
...
import hs.jjm.FanOutOneBlockAsyncDFSOutputHelper.*;
...
public class FanOutOneBlockAsyncDFSOutput implements AsyncFSOutput {
  ...
  private final SendBufSizePredictor
      sendBufSizePRedictor = new SendBufSizePredictor();
  ...
  private void setupReceiver(int timeoutMs) {
    FanOutOneBlockAsyncDFSOutput.AckHandler
        ackHandler = new AckHandler(timeoutMs);
  ...
  FanOutOneBlockAsyncDFSOutput(Configuration conf, DistributedFileSystem dfs, DFSClient client,
    ...
    this.state = FanOutOneBlockAsyncDFSOutput.State.STREAMING;
  ...
  private void flushBuffer(CompletableFuture<Long> future, ByteBuf dataBuf,
    ...
    FanOutOneBlockAsyncDFSOutput.Callback c =
        new FanOutOneBlockAsyncDFSOutput.Callback(future, nextPacketOffsetInBlock + dataLen, datanodeInfoMap.keySet(), dataLen);
  ...
  private void flush0(CompletableFuture<Long> future, boolean syncBlock) {
    if (state != FanOutOneBlockAsyncDFSOutput.State.STREAMING) {
      future.completeExceptionally(new IOException("stream already broken"));
      return;
    }
    ...
    if (dataLen == trailingPartialChunkLength) {
      ...
      FanOutOneBlockAsyncDFSOutput.Callback lastFlush = waitingAckQueue.peekLast();
      if (lastFlush != null) {
        FanOutOneBlockAsyncDFSOutput.Callback
            c = new FanOutOneBlockAsyncDFSOutput.Callback(future, lengthAfterFlush, Collections.emptySet(), dataLen);
        waitingAckQueue.addLast(c);
  ...
  private void endBlock() throws IOException {
    ...
    state = FanOutOneBlockAsyncDFSOutput.State.CLOSING;
    ...
    headerBuf.writerIndex(headerLen);
    CompletableFuture<Long> future = new CompletableFuture<>();
    waitingAckQueue.add(new FanOutOneBlockAsyncDFSOutput.Callback(future, finalizedLength, datanodeInfoMap.keySet(), 0));
  ...
  public void close() throws IOException {
    endBlock();
    state = FanOutOneBlockAsyncDFSOutput.State.CLOSED;
    closeDataNodeChannelsAndAwait();
  ...
  public boolean isBroken() {
    return state == FanOutOneBlockAsyncDFSOutput.State.BROKEN;
  }
