...
public class StreamSlowMonitor implements ConfigurationObserver {
  ...
  private final LoadingCache<DatanodeInfo, Deque<StreamSlowMonitor.PacketAckData>> datanodeSlowDataQueue;
  ...
  public StreamSlowMonitor(Configuration conf, String name,
    ...
    this.datanodeSlowDataQueue = CacheBuilder.newBuilder()
        ...
        .expireAfterWrite(
            ...
            TimeUnit.HOURS)
        .build(new CacheLoader<DatanodeInfo, Deque<StreamSlowMonitor.PacketAckData>>() {
          @Override
          public Deque<StreamSlowMonitor.PacketAckData> load(DatanodeInfo key) throws Exception {
  ...
  public static org.apache.hadoop.hbase.io.asyncfs.monitor.StreamSlowMonitor create(Configuration conf, String name) {
    return new org.apache.hadoop.hbase.io.asyncfs.monitor.StreamSlowMonitor(conf, name, new org.apache.hadoop.hbase.io.asyncfs.monitor.ExcludeDatanodeManager(conf));
  }
  ...
  private boolean addSlowAckData(DatanodeInfo datanodeInfo, long dataLength, long processTime) {
    Deque<StreamSlowMonitor.PacketAckData> slowDNQueue = datanodeSlowDataQueue.getUnchecked(datanodeInfo);
    long current = EnvironmentEdgeManager.currentTime();
    while (
    ...
    slowDNQueue.addLast(new StreamSlowMonitor.PacketAckData(dataLength, processTime));
